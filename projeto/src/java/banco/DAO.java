...
public class DAO {
    ...
    public List<String> listar_pedidos() {
    conectar();
    List<String> pedidos = new ArrayList<>();
    try {
        String sql = "SELECT o.id, r.nome, o.status " +
                     "FROM \"" + schema + "\".orders o " +
                     "JOIN \"" + schema + "\".refeicoes r ON o.id_refeicao = r.id " +
                     "ORDER BY o.id;";
        PreparedStatement pstm = conn.prepareStatement(sql);;
        ResultSet rs = pstm.executeQuery();
        while (rs.next()) {
            String pedido = "ID: " + rs.getInt("id") + 
                            " - Refeição: " + rs.getString("nome") + 
                            " - Status: " + rs.getString("status");
            pedidos.add(pedido);
        }
    } catch (SQLException e) {
        System.out.println("Falha ao listar pedidos: " + e.getMessage());
    }
    desconectar();
    return pedidos;
}

// Listar feedbacks das refeições
public List<String> listar_feedbacks() {
    conectar();
    List<String> feedbacks = new ArrayList<>();
    try {
        String sql = "SELECT f.id, r.nome, f.comentario " +
                     "FROM \"" + schema + "\".feedback f " +
                     "JOIN \"" + schema + "\".refeicoes r ON f.id_refeicao = r.id " +
                     "ORDER BY f.id;";
        PreparedStatement pstm = conn.prepareStatement(sql);
        ResultSet rs = pstm.executeQuery();
        while (rs.next()) {
            String fb = "ID: " + rs.getInt("id") + 
                        " - Refeição: " + rs.getString("nome") + 
                        " - Comentário: " + rs.getString("comentario");
            feedbacks.add(fb);
        }
    } catch (SQLException e) {
        System.out.println("Falha ao listar feedbacks: " + e.getMessage());
    }
    desconectar();
    return feedbacks;
}
